package org.usvplanner.android.fragments;
...
import org.usvplanner.android.activities.helpers.MapPreferencesActivity;
import org.usvplanner.android.fragments.widget.WidgetsListPrefFragment;
import org.usvplanner.android.maps.providers.DPMapProvider;
import org.usvplanner.android.utils.Utils;
import org.usvplanner.android.utils.analytics.GAUtils;
import org.usvplanner.android.utils.prefs.DroidPlannerPrefs;
import org.usvplanner.android.utils.unit.UnitManager;
...
import org.usvplanner.android.utils.unit.systems.UnitSystem;
...
public class SettingsFragment extends PreferenceFragment implements OnSharedPreferenceChangeListener,
        DroidPlannerApp.ApiListener, PreferenceFragment.OnPreferenceStartScreenCallback {
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        setupCreditsPage();
        setupSpeedPreferences();
        setupVoltagePreferences();
        setupMisPreferences();
        setupMisParamPreferences();
        setupServoParamPreferences();
    }
    ...
    //打点时间间隔
    private void setupMisPreferences() {
        final String timeSeekBarPrefKey = DroidPlannerPrefs.PREF_MIS_TIME_NAME;
        final SeekBarPreference timeBarPref = (SeekBarPreference) findPreference(timeSeekBarPrefKey);
        if(timeBarPref != null) {
            timeBarPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    try{
                        Integer timeMisAddPoint = (Integer) newValue;
                        //保存到prefs
                        dpPrefs.setMisTimeValue(timeMisAddPoint);
                    }
                    catch (Exception e) {
    ...
    //MIS_RESTART参数设置各项
    private void setupMisParamPreferences() {
        final ListPreference misRestart = (ListPreference) findPreference("mis_sel_key");
        if(misRestart != null) {
            misRestart.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
//                    Toast.makeText(getContext(), "Sel:" + newValue.toString(), Toast.LENGTH_SHORT).show();
                    try{
                        int value = Integer.parseInt(newValue.toString());
//                        switch (value) {
//                            case 0:
//                                misRestart.setSummary("恢复任务");
//                                break;
//                            case 1:
//                                misRestart.setSummary("重新开始");
//                                break;
//                        }
                        setupParame("MIS_RESTART", value);
                    }
                    catch (Exception e) {
                        Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
    ...
    //舵机参数设置
    private void setupServoParamPreferences() {
        final SeekBarPreference servo5_bar = (SeekBarPreference) findPreference("servo5_bar_key");
        final SeekBarPreference servo6_bar = (SeekBarPreference) findPreference("servo6_bar_key");
        final SeekBarPreference servo7_bar = (SeekBarPreference) findPreference("servo7_bar_key");

        if(servo5_bar != null) {
            servo5_bar.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    try{
                        Integer value = (Integer) newValue;
                        //修改参数
                        setupParame("SERVO5_SPEED", value);
                    }
                    catch (Exception e) {
                        Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
        ...
        if(servo6_bar != null) {
            servo6_bar.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    try{
                        Integer value = (Integer) newValue;
                        //修改参数
                        setupParame("SERVO6_SPEED", value);
                    }
                    catch (Exception e) {
                        Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
        ...
        if(servo7_bar != null) {
            servo7_bar.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    try{
                        Integer value = (Integer) newValue;
                        //修改参数
                        setupParame("SERVO7_SPEED", value);
                    }
                    catch (Exception e) {
                        Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
    ...
    private void setupParame(String par,int value) {
        ...
        if (drone != null && drone.isConnected()) {
            ...
            if (droneParams != null) {
                ...
                for (int i = 0; i < parametersList.size(); i++) {
                    if (parametersList.get(i).getName().equals(par)) {
                        //获取type
                        type = parametersList.get(i).getType();
                        List<Parameter> changeParameterList = new ArrayList<Parameter>();
                        //写入参数
                        Parameter newparameter = new Parameter(par, value, type);
                        changeParameterList.add(newparameter);
    ...
    private void setupVoltagePreferences() {

        final EditTextPreference minVoltagePref = (EditTextPreference) findPreference(DroidPlannerPrefs.PREF_VOLTAGE_MIN_VALUE);
        if (minVoltagePref != null) {
            final float minVoltage = DroidPlannerPrefs.getInstance(getContext()).getMinVoltage();
            minVoltagePref.setText(String.format(Locale.US, "%2.1f", minVoltage));
            minVoltagePref.setSummary(String.format(Locale.ENGLISH, "%2.1f V", minVoltage));

            minVoltagePref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    final Context context = getContext();
                    try {
                        final float voltageValue = Float.parseFloat(newValue.toString());
                        minVoltagePref.setText(String.format(Locale.US, "%2.1f", voltageValue));
                        minVoltagePref.setSummary(String.format(Locale.ENGLISH, "%2.1f V", voltageValue));
                        dpPrefs.setVoltageMinValue(voltageValue);

                    }catch(NumberFormatException e) {
                        if (context != null) {
                            Toast.makeText(context, "无效的电压值", Toast.LENGTH_LONG).show();
                        }
                    }
                    return false;
                }
