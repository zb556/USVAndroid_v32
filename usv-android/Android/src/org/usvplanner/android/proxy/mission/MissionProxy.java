package org.usvplanner.android.proxy.mission;
...
public class MissionProxy implements DPMap.PathSource {
    ...
    public MissionItem[] getMissionItems() {
    ...
    public void sendMissionToAPM(Drone drone) {
        ...
        if (missionItemsCount > 0) {
            ...
            for (MissionItemProxy itemProxy : missionItemProxies) {
                ...
                missionItemsList += itemProxy.getMissionItem().getType().getLabel2();
            }
        }
    ...
    public void readMissionFromFile(final Uri fileUri){
        MissionApi.getApi(drone).loadAndSetMission(fileUri, new MissionApi.LoadingCallback<Mission>() {
            ...
            public void onLoadingStart() {
                Toast.makeText(context, R.string.load_mission, Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onLoadingComplete(Mission loaded) {
                load(loaded);
                Toast.makeText(context, R.string.mission_loaded, Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onLoadingFailed() {
                Toast.makeText(context, R.string.mission_load_failed, Toast.LENGTH_SHORT).show();
            }
        });
    }
}
