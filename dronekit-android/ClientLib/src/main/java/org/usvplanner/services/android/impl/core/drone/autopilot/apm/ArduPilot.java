...
import org.usvplanner.services.android.impl.core.MAVLink.MavLinkParameters;
import org.usvplanner.services.android.impl.core.MAVLink.WaypointManager;
import org.usvplanner.services.android.impl.core.MAVLink.command.doCmd.MavLinkDoCmds;
...
import org.usvplanner.services.android.impl.core.drone.autopilot.apm.variables.APMHeartBeat;
import org.usvplanner.services.android.impl.core.drone.autopilot.generic.GenericMavLinkDrone;
import org.usvplanner.services.android.impl.core.drone.variables.ApmModes;
...
import org.usvplanner.services.android.impl.core.drone.variables.HeartBeat;
import org.usvplanner.services.android.impl.core.drone.variables.Magnetometer;
import org.usvplanner.services.android.impl.core.drone.variables.RC;
...
import org.usvplanner.services.android.impl.utils.CommonApiUtils;
 ...
 */
public abstract class ArduPilot extends GenericMavLinkDrone implements DroneInterfaces.OnWaypointManagerListener {
    ...
    private final org.usvplanner.services.android.impl.core.drone.variables.RC rc;
    ...
    private final LevelCalibration levelCalibration;
    ...
    public ArduPilot(String droneId, Context context, DataLink.DataLinkProvider<MAVLinkMessage> mavClient,
        ...
        this.waypointManager.setWaypointManagerListener(this);
        ...
        this.levelCalibration = new LevelCalibration(this, handler);
    ...
    protected void setMissionUploadPro(int index, int count) {
        if(count > 0) {
            missionUploadPro.setIndex(index);
            missionUploadPro.setCount(count);
            notifyDroneEvent(DroneInterfaces.DroneEventsType.MISSION_WP_UP);
        }
    }
    ...
    public boolean executeAsyncAction(Action action, final ICommandListener listener) {
        ...
        switch (type) {
            ...
            case CalibrationActions.ACTION_START_LEVEL_CALIBRATION:
                CommonApiUtils.startLevelCalibration(this, listener);
                return true;
...
//                    rc.setRcChannelsInputValues((msg_rc_channels) message);
                    msg_rc_channels channels = (msg_rc_channels) message;
                    int chroll = 0;
                    int chpitch = 0;
                    int chthro = 0;
                    int chyaw = 0;
                    if (!getRcmapRoll()) {
                        chroll = channels.chan1_raw;
                        chpitch = channels.chan2_raw;
                        chthro = channels.chan3_raw;
                        chyaw = channels.chan4_raw;
                    } else {
                        try {
                            int[] chan = getRcChannelSetup();
                            switch (chan[0]) {
                                case 1:
                                    chroll = channels.chan1_raw;
                                    break;
                                case 2:
                                    chroll = channels.chan2_raw;
                                    break;
                                case 3:
                                    chroll = channels.chan3_raw;
                                    break;
                                case 4:
                                    chroll = channels.chan4_raw;
                                    break;
                            }
                            switch (chan[1]) {
                                case 1:
                                    chpitch = channels.chan1_raw;
                                    break;
                                case 2:
                                    chpitch = channels.chan2_raw;
                                    break;
                                case 3:
                                    chpitch = channels.chan3_raw;
                                    break;
                                case 4:
                                    chpitch = channels.chan4_raw;
                                    break;
                            }
                            switch (chan[2]) {
                                case 1:
                                    chthro = channels.chan1_raw;
                                    break;
                                case 2:
                                    chthro = channels.chan2_raw;
                                    break;
                                case 3:
                                    chthro = channels.chan3_raw;
                                    break;
                                case 4:
                                    chthro = channels.chan4_raw;
                                    break;
                            }
                            switch (chan[3]) {
                                case 1:
                                    chyaw = channels.chan1_raw;
                                    break;
                                case 2:
                                    chyaw = channels.chan2_raw;
                                    break;
                                case 3:
                                    chyaw = channels.chan3_raw;
                                    break;
                                case 4:
                                    chyaw = channels.chan4_raw;
                                    break;
                    ...
                    processRC_Channels(chroll, chpitch, chthro, chyaw,
    ...
    private boolean getRcmapRoll() {
        boolean get = false;
        Parameter rcmaproll = getParameterManager().getParameter("RCMAP_ROLL");
        if (rcmaproll == null) {
            get = false;
        } else {
            get = true;
    ...
    private int[] getRcChannelSetup() {
        Parameter chroll = getParameterManager().getParameter("RCMAP_ROLL");
        Parameter chpitch = getParameterManager().getParameter("RCMAP_PITCH");
        Parameter chthro = getParameterManager().getParameter("RCMAP_THROTTLE");
        Parameter chyaw = getParameterManager().getParameter("RCMAP_YAW");
        if (chroll == null || chpitch == null || chthro == null || chyaw == null) {
            return null;
        }
        return new int[]{(int) chroll.getValue(), (int) chpitch.getValue(), (int) chthro.getValue(), (int) chyaw.getValue()};
    }
